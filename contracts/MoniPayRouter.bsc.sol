...
/**
 * MoniPayRouter for BSC Mainnet (Chain ID: 56)
 * Deploy with constructor arg: platformTreasury = 0xfa2B8eD012f756E22E780B772d604af4575d5fcf
 * 
 * Changes from Base version:
 * - USDC replaced with USDT (0x55d398326f99059fF775485246999027B3197955) — 18 decimals
 * - EIP712 domain name remains "MoniPay Router" version "1"
 */
...
/**
 * @title MoniPayRouter
 * @notice Gasless payment router for USDT on BSC
 */
contract MoniPayRouter is EIP712, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using ECDSA for bytes32;

    // BSC Mainnet USDT Address (18 decimals)
    ...
    function relayPayment(
        ...
        if (USDT.allowance(from, address(this)) < totalAmount) revert InsufficientAllowance();
        if (USDT.balanceOf(from) < totalAmount) revert InsufficientBalance();

        USDT.safeTransferFrom(from, to, amount);
        USDT.safeTransferFrom(from, platformTreasury, fee);
