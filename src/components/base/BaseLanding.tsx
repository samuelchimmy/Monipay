 ...
 * recolored to Base's blue palette and USDC copy. Strictly Base.
 */
...
const baseLogoLight = '/chains/base-logo-blue.svg';
const baseLogoDark = '/chains/base-logo.svg';
...
const BaseLogo = ({ className = '' }: { className?: string }) => (
  <>
    <img src={baseLogoLight} alt="Base" className={`${className} block dark:hidden`} />
    <img src={baseLogoDark} alt="Base" className={`${className} hidden dark:block`} />
  </>
);
...
/* ── Header ── */
function BaseHeader({ onSignIn }: { onSignIn: () => void }) {
      ...
      <div className="mx-auto max-w-5xl">
        ...
        >
          <div className="flex items-center gap-2 pl-1">
            ...
            <span className="font-bold tracking-tight text-[15px]" style={{ color: 'hsl(var(--mp-ink))' }}>Base</span>
          </div>
...
function BaseHero({ onGetStarted }: { onGetStarted: () => void }) {
  return (
      ...
      <div className="relative z-10 mx-auto max-w-5xl text-center">
        ...
        <Reveal delay={0.05}>
          <h1 className="mt-5 text-[30px] sm:text-[44px] md:text-[54px] leading-[1.04] font-extrabold tracking-tight" style={{ color: 'hsl(var(--mp-ink))' }}>
            ...
            <br />
            <span style={{ color: 'hsl(var(--mp-primary))' }}>Powered by Base.</span>
          </h1>
        </Reveal>
        <Reveal delay={0.1}>
          <p className="mx-auto mt-4 max-w-[600px] text-[15px] sm:text-base" style={{ color: 'hsl(var(--mp-muted))' }}>
            Send USDC to anyone just by posting and chatting on X, Discord, Telegram, or Bluesky. No addresses. No gas. No friction. Just chat and tip.
          </p>
        </Reveal>
...
const CHAT_DEMOS = [
  { network: 'X',        Icon: XIcon, handle: '@alex',
    ...
    bot: 'Done. $5 USDC sent to @alex on Base. Gas sponsored. Receipt 🔗',
    seed: 'alex-x' },
  { network: 'Discord',  img: discordSvg, handle: '#design-team',
    user: '@MoniBot split $40 between @lara, @sam, @jude for lunch',
    bot: 'Sent $13.33 USDC to 3 recipients on Base. One transaction, zero fees.',
    seed: 'mara-d' },
  { network: 'Telegram', img: telegramSvg, handle: '@nora',
    user: '@MoniBot pay @nora $10 for the design',
    bot: 'Paid. $10 USDC delivered on Base. Nora does not need a MoniPay account yet. MagicPay handled it.',
    seed: 'nora-t' },
...
const SOCIAL_CARDS = [
  { Icon: Send,          name: 'CasualPay',
    ...
    desc: 'Post a standalone command, a quote post, or a reply: "@MoniPay send $5 to @alex". The tip lands instantly in their MoniPay wallet on Base.' },
  ...
  { Icon: AtSign,        name: 'TagPay',
    ...
    desc: 'Send to a username instead of a wallet string. Your @MoniTag is your identity for receiving USDC on Base.' },
  { Icon: CalendarClock, name: 'SchedulePay',
    headline: 'Send later, automatically',
    desc: 'Queue a future payment from chat: "@MoniBot pay @kai $200 on Friday". MoniPay holds the instruction and executes on Base when the time arrives.' },
  { Icon: Repeat2,       name: 'SubscriptionPay',
    headline: 'Recurring payments by chat',
    desc: 'Subscribe with a single command: "@MoniBot subscribe me to @cryptoalpha for 1 month". Auto-renews on Base until you cancel.' },
];
...
/* ── Chain stats ── */
const STATS = [
  { Icon: Zap,        title: 'Instant Finality', stat: '~2s',  desc: 'Sub-2s blocks on Base. Confirmations land before the keyboard catches up.' },
  { Icon: Shield,     title: 'Coinbase-secured', stat: 'OP Stack', desc: 'Built on Ethereum security via the OP Stack and Coinbase infrastructure.' },
  { Icon: DollarSign, title: 'Native USDC',     stat: 'USDC', desc: 'Circle-issued USDC native to Base. The deepest stablecoin liquidity on any L2.' },
  { Icon: Smartphone, title: 'Mobile-Ready',    stat: '100M+', desc: 'Distribution through the Coinbase ecosystem and 100M+ verified users.' },
];
      ...
      <div className="mx-auto max-w-6xl">
        <Reveal className="text-center mb-10">
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight" style={{ color: 'hsl(var(--mp-ink))' }}>
            Built on Base for a reason.
          </h2>
          <p className="mt-3 text-base max-w-lg mx-auto" style={{ color: 'hsl(var(--mp-muted))' }}>
            Fast, cheap, secure, and USDC-native. The right rails for everyday money.
          </p>
        </Reveal>
          ...
          <div className="grid sm:grid-cols-2 rounded-3xl overflow-hidden" style={{ border: '1px solid hsl(var(--mp-border))' }}>
            ...
            <div className="p-7" style={{ background: 'hsl(var(--mp-primary))', color: 'white' }}>
              <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest opacity-90">
                <Check className="h-3 w-3" /> MoniPay on Base
              </div>
...
/* ── Platform features ── */
const PLATFORM = [
  { Icon: Bot,        title: 'MoniBot AI Agent',  desc: 'Autonomous AI executing USDC transfers across X, Discord, Telegram, and Bluesky on Base.' },
  { Icon: Store,      title: 'Merchant Suite',    desc: 'POS, product catalog, invoicing, and payment links for Base USDC merchants.' },
  { Icon: Users,      title: 'P2P Transfers',     desc: 'Send USDC to any @MoniTag on Base. No wallet addresses, no mistakes.' },
  { Icon: Smartphone, title: 'Base-Native Rails', desc: 'Built on Base primitives: OP Stack security, native USDC, gas sponsored by MoniPay’s paymaster.' },
  { Icon: CreditCard, title: 'Payment Gateway',   desc: 'Stripe-style API for online USDC payments on Base with merchant webhooks.' },
...
/* ── How it works ── */
const STEPS = [
  { num: '01', title: 'Open MoniPay on Base', desc: 'Create your @MoniTag in seconds. A wallet is generated locally and encrypted with your PIN.' },
  ...
  { num: '04', title: 'Cash out anywhere', desc: 'Spend in MoniPay or withdraw your USDC to any wallet you control.' },
];
      ...
      <Reveal className="mx-auto max-w-5xl">
        <div className="relative overflow-hidden rounded-[32px] p-8 sm:p-14 text-center" style={{ background: 'hsl(var(--mp-primary))' }}>
          ...
          <p className="relative mt-3 text-white/85 max-w-md mx-auto">
            Open MoniPay on Base and pay anyone, anywhere, just by chatting.
          </p>
          <div className="relative mt-7 flex flex-row gap-3 items-center justify-center flex-wrap">
            ...
            <a
              href="https://docs.monipay.xyz/chains/base"
              target="_blank"
              rel="noreferrer"
            ...
            >
              Read Base docs <ArrowUpRight className="h-4 w-4" />
            </a>
          </div>
        </div>
      </Reveal>
    ...
    <footer className="px-6 lg:px-16 py-10 border-t" style={{ borderColor: 'hsl(var(--mp-border))' }}>
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold tracking-tight text-[13px]" style={{ color: 'hsl(var(--mp-ink))' }}>Base</span>
        </div>
...
export function BaseLanding({
  ...
  return (
    <div data-base className="min-h-screen overflow-x-hidden">
      <BaseHeader onSignIn={handleSignIn} />
      <BaseHero onGetStarted={handleGetStarted} />
      <SocialPaymentCards />
      <ChainStats />
      <FeeCompare />
      <PlatformGrid />
