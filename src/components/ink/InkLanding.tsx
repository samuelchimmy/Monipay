...
const inkLogo = '/chains/ink-logo.webp';
...
/* Ink brand mark — uploaded glyph from /chains/ink-logo.webp
 * to a solid ink in light mode and solid white in dark mode. */
const InkLogo = ({ className = '' }: { className?: string }) => (
  <img
    src={inkLogo}
    alt="Ink"
...
/* ── Header ── */
function InkHeader({ onSignIn }: { onSignIn: () => void }) {
      ...
      <div className="mx-auto max-w-5xl">
        ...
        >
          <div className="flex items-center gap-2 pl-1">
            ...
            <InkLogo className="h-5" />
            <span className="font-bold tracking-tight text-[15px]" style={{ color: 'hsl(var(--mp-ink))' }}>Ink</span>
          </div>
...
function InkHero({ onGetStarted }: { onGetStarted: () => void }) {
  return (
      ...
      <div className="relative z-10 mx-auto max-w-5xl text-center">
        ...
        <Reveal delay={0.05}>
          ...
          >
            {'Payments, native.'}
            <br />
            <span style={{ color: 'hsl(var(--mp-primary))' }}>{'Powered by Ink.'}</span>
          </h1>
        </Reveal>
        <Reveal delay={0.1}>
          <p
          ...
          >
            {'Send USDT0 to anyone just by posting and chatting on X, Discord, Telegram, or Bluesky. Built on Ink, Kraken’s Layer 2 on the Optimism Superchain. No addresses. No gas. No friction.'}
          </p>
        </Reveal>
        ...
        <Reveal delay={0.15} className="mt-6 flex flex-col items-center gap-2">
          <button
            ...
            style={{ background: 'hsl(var(--mp-primary))', boxShadow: '0 14px 32px -10px hsl(var(--mp-primary) / 0.55)' }}
          >
            {'Get started'}
            <ArrowRight className="h-4 w-4" />
          </button>
          <span className="text-[12px]" style={{ color: 'hsl(var(--mp-muted))' }}>
            {'Tap in. Your wallet is created and funded in seconds.'}
          </span>
        </Reveal>
...
const CHAT_DEMOS = [
  {
    ...
    bot: 'Done. $5 USDT0 sent to @alex on Ink. Gas sponsored. Receipt 🔗',
    seed: 'alex-x',
  },
  {
    network: 'Discord',
    ...
    bot: 'Sent $13.33 USDT0 to 3 recipients on Ink. One transaction, zero fees.',
    seed: 'mara-d',
  },
  {
    network: 'Telegram',
    ...
    bot: 'Paid. $10 USDT0 delivered on Ink. Nora does not need a MoniPay account yet. MagicPay handled it.',
    seed: 'nora-t',
  },
];
...
const SOCIAL_CARDS = [
  { Icon: Send,          name: 'CasualPay',       headlineKey: 'Tip any MoniPay user by their @MoniTag',       descKey: 'Post a standalone command, a quote post, or a reply: "@MoniPay send $5 to @alex". The tip lands instantly in their MoniPay wallet on Ink.' },
  ...
  { Icon: Users,         name: 'GroupPay',        headlineKey: 'Pay everyone at once',        descKey: 'One command, many recipients. Try "@MoniBot split $30 between @lara, @sam, @jude" or run a giveaway: "@MoniBot drop $50 to the next 25 repliers". Atomic in a single Ink transaction.' },
  { Icon: AtSign,        name: 'TagPay',          headlineKey: 'One @MoniTag, no addresses',          descKey: 'Send to a username instead of a wallet string. Your @MoniTag is your identity for receiving USDT0 on Ink.' },
  { Icon: CalendarClock, name: 'SchedulePay',     headlineKey: 'Send later, automatically',     descKey: 'Queue a future payment from chat: "@MoniBot pay @kai $200 on Friday". MoniPay holds the instruction and executes on Ink when the time arrives.' },
  { Icon: Repeat2,       name: 'SubscriptionPay', headlineKey: 'Recurring payments by chat', descKey: 'Subscribe with a single command: "@MoniBot subscribe me to @cryptoalpha for 1 month". Auto-renews on Ink until you cancel.' },
];
...
/* ── Chain stats ── */
const STATS = [
  { Icon: Zap,        titleKey: 'Instant Finality', stat: '~5s',   descKey: '~1s blocks on Ink. Confirmations land before the keyboard catches up.' },
  { Icon: Leaf,       titleKey: 'USDT0-Native',  statKey: 'USDT0', descKey: 'Native USDT0 stablecoin on Ink. Deep liquidity from Kraken’s ecosystem.' },
  { Icon: DollarSign, titleKey: 'Sub-Cent Gas',    stat: '<$0.01',  descKey: 'Fees so small they vanish. MoniPay’s paymaster sponsors them anyway, so users pay zero.' },
  { Icon: Smartphone, titleKey: 'OP Superchain',  stat: 'OP Stack',   descKey: 'Built by Kraken on the OP Stack. Bridged liquidity across the Superchain ecosystem.' },
];
      ...
      <div className="mx-auto max-w-6xl">
        <Reveal className="text-center mb-10">
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight" style={{ color: 'hsl(var(--mp-ink))' }}>
            {'Built on Ink for a reason.'}
          </h2>
          <p className="mt-3 text-base max-w-lg mx-auto" style={{ color: 'hsl(var(--mp-muted))' }}>
            {'Kraken’s Layer 2 on the Optimism Superchain. Sub-cent gas, USDT0 settlement, and the security of OP Stack.'}
          </p>
        </Reveal>
          ...
          >
            ...
            <div className="p-7" style={{ background: 'hsl(var(--mp-primary))', color: 'white' }}>
              <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest opacity-90">
                <Check className="h-3 w-3" /> {'MoniPay on Ink'}
              </div>
...
/* ── Platform features (Ink only) ── */
const PLATFORM = [
  { Icon: Bot,        titleKey: 'MoniBot AI Agent',     descKey: 'Autonomous AI executing USDT0 transfers across X, Discord, Telegram, and Bluesky on Ink.' },
  { Icon: Store,      titleKey: 'Merchant Suite',   descKey: 'POS, product catalog, invoicing, and payment links for Ink USDT0 merchants.' },
  { Icon: Users,      titleKey: 'P2P Transfers',     descKey: 'Send USDT0 to any @MoniTag on Ink. No wallet addresses, no mistakes.' },
  { Icon: Smartphone, titleKey: 'Ink-Native Rails', descKey: 'Built on Ink primitives: OP Stack security, USDT0 settlement, gas sponsored by MoniPay’s paymaster.' },
  { Icon: CreditCard, titleKey: 'Payment Gateway', descKey: 'Stripe-style API for online USDT0 payments on Ink with merchant webhooks.' },
...
const STEPS = [
  ...
  { num: '03', titleKey: 'Reply to pay', descKey: 'Just type "@MoniPay send $5 to @alex" on any supported network. We route it through Ink.' },
      ...
      <Reveal className="mx-auto max-w-5xl">
        ...
        >
          ...
          <h2 className="relative text-3xl sm:text-5xl font-extrabold tracking-tight text-white">
            {'The social wallet for Ink.'}
          </h2>
          <p className="relative mt-3 text-white/85 max-w-md mx-auto">
            {'MoniPay is live on Ink. Create a @MoniTag, fund with USDT0, and start sending in seconds.'}
          </p>
          <div className="relative mt-7 flex flex-col gap-3 items-center justify-center">
            ...
            >
              {'Read Ink docs'} <ArrowUpRight className="h-4 w-4" />
            </a>
          </div>
        </div>
      </Reveal>
...
/* ── Footer (Ink-only, no chain cross-links) ── */
function InkFooter() {
...
export function InkLanding({
  ...
  useEffect(() => {
    document.documentElement.setAttribute('data-ink-route', 'true');
    return () => document.documentElement.removeAttribute('data-ink-route');
  }, []);

  return (
    <div data-ink className="min-h-screen overflow-x-hidden">
      <InkHeader onSignIn={onSignIn} />
      <InkHero onGetStarted={onGetStarted} />
      <SocialPaymentCards />
      <ChainStats />
      <FeeCompare />
      <PlatformGrid />
