/**
 * MiniPaySignInChoice.tsx
 * Sign-in chooser styled to match the /minipay landing aesthetic.
 * Functional parity with src/components/SignInChoice.tsx — only visuals change.
 */
import { motion } from 'framer-motion';
import { useTheme } from 'next-themes';
import { useTranslation } from 'react-i18next';
import { ArrowLeft, KeyRound, UserRoundPlus, ArrowUpRight, Sun, Moon } from 'lucide-react';
import { MoniPayLogo } from '@/components/MoniPayLogo';
import { Footer } from '@/components/Footer';
import { feedback } from '@/lib/feedback';

interface MiniPaySignInChoiceProps {
  onImportWallet: () => void;
  onGoogleSignIn: () => void;
...
export function MiniPaySignInChoice({
  onImportWallet,
  onGoogleSignIn,
  onCreateAccount,
  onBack,
}: MiniPaySignInChoiceProps) {
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 sm:px-6">
        <div className="max-w-md mx-auto w-full flex flex-col h-full justify-center">
          ...
          >
            ...
            >
              {t('minipay_signin_welcome_prefix')}{' '}
          ...
          >
            ...
            >
              <div className="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-[inset_0_0_0_1px_hsl(var(--primary-foreground)/0.18)]" style={{ background: 'hsl(var(--primary-foreground) / 0.15)' }}>
                <KeyRound className="w-5 h-5" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-[15px] leading-tight">{t('minipay_signin_import_title')}</p>
                <p className="text-[12px] opacity-80 mt-0.5">{t('minipay_signin_import_desc')}</p>
              </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold text-[15px] leading-tight" style={{ color: 'hsl(var(--mp-ink))' }}>
                  {t('minipay_signin_create_title')}
                </p>
                <p className="text-[12px] mt-0.5" style={{ color: 'hsl(var(--mp-muted))' }}>
                  {t('minipay_signin_create_desc')}
                </p>
              </div>
            </button>
          </motion.div>
